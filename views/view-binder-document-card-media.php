<?php
/**
 * View Binder Document Card Media
 *
...
$size     = $document->size;
$image    = $document->get_thumbnail( $document->binder_id, 'medium' );
...
if ( ! empty( $document_meta ) ) {
	...
	$image_src = get_the_post_thumbnail_url( $document_meta['post_id'], 'medium' );
	if ( empty( $image_src ) ) {
	...
	<div class="c-binder-card-wrapper">
		<article class="o-media | c-binder-card c-binder-card--media">
			<?php
			if ( ! empty( $image_src ) ) {
				?>
				<div class="o-media__img" style="background-image: url('<?php echo esc_url( $image_src );?>');">
				</div>
				<?php
			}
			?>
